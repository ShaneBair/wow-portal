...
export type PortableHoleBoostStatus = "pending" | "sent" | "failed" | "unknown";

export interface PortableHoleBoostRecord {
  requestId: string;
  boostKey: string;
  accountId: number;
  ...
  itemEntry: number;
  itemCount: number;
  status: PortableHoleBoostStatus;
  createdAt: Date;
}

export interface ReservePortableHoleBoostInput {
  requestId: string;
  boostKey: string;
  accountId: number;
  characterGuid: number;
...
export type ReservePortableHoleBoostResult =
  | { kind: "inserted"; record: PortableHoleBoostRecord }
  | { kind: "existing"; record: PortableHoleBoostRecord }
  | { kind: "conflict" };

export type PortableHoleMailMatch = "exact" | "absent" | "ambiguous";

export class PortableHoleBoostRepositoryError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "PortableHoleBoostRepositoryError";
  }
}
...
function tableNames(config: Pick<PortalDatabaseConfig, "charactersDatabase" | "stateDatabase">) {
  ...
  return {
    requests: qualified(stateDatabase, "portable_hole_boost_requests"),
...
function readInteger(value: unknown, key: string): number {
  ...
  if (typeof parsed !== "number" || !Number.isSafeInteger(parsed) || parsed < 0) {
    throw new PortableHoleBoostRepositoryError(`Portable Hole boost ${key} is invalid.`);
  }
  return parsed;
}
...
function readDate(value: unknown): Date {
  ...
  if (!Number.isFinite(date.getTime())) {
    throw new PortableHoleBoostRepositoryError("Portable Hole boost creation timestamp is invalid.");
  }
  return date;
}

function mapRecord(row: unknown): PortableHoleBoostRecord {
  if (typeof row !== "object" || row === null || Array.isArray(row)) {
    throw new PortableHoleBoostRepositoryError("Portable Hole boost request row is invalid.");
  }
  ...
  if (
    ...
    throw new PortableHoleBoostRepositoryError("Portable Hole boost request material is invalid.");
  }
  return {
    ...
    status: source.status as PortableHoleBoostStatus,
...
function requestLockKey(requestId: string): string {
  return `wow_portal_holes_${requestId}`;
}
...
async function acquireLock(connection: PoolConnection, key: string): Promise<void> {
  ...
  if (readInteger(rows[0]?.acquired, "request lock") !== 1) {
    throw new PortableHoleBoostRepositoryError("Portable Hole boost request lock is unavailable.");
  }
}
...
export class PortableHoleBoostRepository {
  async reserve(input: ReservePortableHoleBoostInput): Promise<ReservePortableHoleBoostResult> {
...
FOR UPDATE`,
      ...
      if (existingRows.length > 1) {
        throw new PortableHoleBoostRepositoryError("Portable Hole boost request ID is ambiguous.");
      }
      if (existingRows.length === 1) {
  ...
  async mark(
    requestId: string,
    status: Exclude<PortableHoleBoostStatus, "pending">,
    resultCategory: string
  ): Promise<void> {
    if (!/^[a-z_]{1,32}$/u.test(resultCategory)) {
      throw new PortableHoleBoostRepositoryError("Portable Hole boost result category is invalid.");
    }
    const { pool, config } = getPortalDatabase();
    const { requests } = tableNames(config);
...
WHERE request_id = ?
  AND status IN ('pending', 'unknown')`,
    ...
    if (result.affectedRows > 1) {
      throw new PortableHoleBoostRepositoryError("Portable Hole boost update affected an invalid number of rows.");
    }
  }

  async inspectMatchingMail(
    record: PortableHoleBoostRecord,
    subject: string,
    body: string
  ): Promise<PortableHoleMailMatch> {
    const { pool, config } = getPortalDatabase();
...
LIMIT 2`,
    ...
    return attachmentCount === record.itemCount && matchingCount === record.itemCount
      ? "exact"
      : "ambiguous";
  }

  async findStalePending(accountId: number, staleAfterMs: number): Promise<PortableHoleBoostRecord[]> {
    const { pool, config } = getPortalDatabase();
    const { requests } = tableNames(config);
    const [rows] = await pool.execute<RowDataPacket[]>({
      sql: `SELECT ${selectColumns()}
...
export const portableHoleBoostRepository = new PortableHoleBoostRepository();
