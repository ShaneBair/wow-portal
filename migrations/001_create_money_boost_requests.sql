...
CREATE TABLE IF NOT EXISTS money_boost_requests (
    ...
    KEY ix_money_boost_requests_account_day (account_id, created_at, status),
    KEY ix_money_boost_requests_pending (status, created_at),
    CONSTRAINT ck_money_boost_requests_gold CHECK (gold >= 1),
    CONSTRAINT ck_money_boost_requests_copper CHECK (copper >= 10000)
